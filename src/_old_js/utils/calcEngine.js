...
export function calculateParams(input, a) {
    const notes = [];
    // Validazioni minime
    ...
    const runnerFactor = 0.00; // per ora zero (non conosciamo i canali); puoi impostare 0.05..0.15 se serve
    ...
    if (tMin < 1.0)
        baseSpeed = 18; // pezzo sottile → serve spingere
    else if (tMin > 3.0)
        baseSpeed = 12; // pezzo spesso → più lento
    ...
    switch (input.material) {
        ...
        case "PC/ABS":
            packPressure_bar = 700;
            break;
        default: packPressure_bar = 600;
    }
    if (a.thickness_mm.max > 3)
        packPressure_bar *= 1.05; // zone spesse → leggero aumento
    ...
    if (defectSet.size)
        notes.push(`Correzioni applicate per difetti: ${Array.from(defectSet).join(", ")}`);
    for (const d of defectSet) {
        const fx = DEFECT_FIX[d];
        if (!fx)
            continue;
        if (fx.speedMul)
            injectionSpeed_cc_s *= fx.speedMul;
        if (fx.packMul)
            packPressure_bar = Math.round(packPressure_bar * fx.packMul);
        if (fx.coolingAddS)
            cooling_s = +(cooling_s + fx.coolingAddS).toFixed(1);
        if (fx.moldTempShift) {
