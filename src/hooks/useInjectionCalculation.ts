...
interface UseInjectionCalculationProps {
  params: InjectionParams;
  onResult: (result: InjectionResult) => void;
  userTouched: { [key: string]: boolean };
}

export function useInjectionCalculation({ params, onResult, userTouched }: UseInjectionCalculationProps) {
  const calculateInjection = useCallback(() => {
    ...
    const viscosityFactor = (() => {
      switch (materiale) {
        case "PP": case "PE": case "PS": return 0.8;
        case "PC": case "PA-GF": case "PBT": return 1.3;
        case "PMMA": case "POM": return 1.1;
    ...
    if (tappe === 1) {
      ...
      steps.push(
        { velocita: Math.round(vel1), posizione: pos1, pressione: press1 },
        { velocita: Math.round(vel2), posizione: 0, pressione: press2 }
      );
    } else { // 3 steps
      ...
      steps.push(
        { velocita: Math.round(vel1), posizione: pos1, pressione: press1 },
        { velocita: Math.round(vel2), posizione: pos2, pressione: press2 },
        { velocita: Math.round(vel3), posizione: 0, pressione: press3 }
      );
    }
  ...
  }, [calculateInjection, params.volumeCarica, params.cuscino, params.margine]);
}
