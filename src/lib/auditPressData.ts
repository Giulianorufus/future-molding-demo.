...
export const AUDIT_PRESSES: Press[] = [
  {
    id: "series-170",
    uiLabel: "Serie 170",
    models: [
...
export const getAuditPressById = (id?: string) => AUDIT_PRESSES.find((p) => p.id === id);
export const getAuditModelsForPress = (pressId?: string) => getAuditPressById(pressId)?.models ?? [];
export const getAuditModelById = (pressId?: string, modelId?: string) => getAuditModelsForPress(pressId).find((m) => m.id === modelId);
