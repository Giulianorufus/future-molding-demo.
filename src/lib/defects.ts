...
export type MachineLimits = {
  maxInjectionSpeed_cm3s?: number;
  maxInjectionPressure_bar?: number;
  clampForce_kN?: number;
  minCushion_cm3?: number;
};

export type MaterialRanges = {
  meltRange_C?: [number, number];
  moldRange_C?: [number, number];
};

export type CalcParams = Record<string, any>; // compatibile con il tuo result di calcolo
...
function clampToMachineAndMaterial(p: CalcParams, ml: MachineLimits, mr: MaterialRanges, notes: string[]) {
  // Velocità iniezione
  if (Array.isArray(p.injSpeedSteps) && ml.maxInjectionSpeed_cm3s) {
    p.injSpeedSteps = p.injSpeedSteps.map((s: number) =>
      clampNum(s, 0, ml.maxInjectionSpeed_cm3s!)
    );
  ...
  if (typeof p.injPressure_bar === 'number' && ml.maxInjectionPressure_bar) {
    ...
    if (p.injPressure_bar < before) notes.push('Pressione limitata dal max macchina');
  }
  if (Array.isArray(p.packPressures_bar) && ml.maxInjectionPressure_bar) {
    p.packPressures_bar = p.packPressures_bar.map((pp: number) =>
      clampNum(pp, 0, ml.maxInjectionPressure_bar!)
    );
  }
...
function addPercent(p: CalcParams, key: string, deltaPct: number) {
  if (typeof p[key] === 'number') p[key] = p[key] * (1 + deltaPct / 100);
}
function addSeconds(p: CalcParams, key: string, deltaS: number) {
  if (typeof p[key] === 'number') p[key] = p[key] + deltaS;
}
function addAbsolute(p: CalcParams, key: string, deltaAbs: number) {
  if (typeof p[key] === 'number') p[key] = p[key] + deltaAbs;
}
function shiftSwitchVolume(p: CalcParams, deltaPctOfShot: number) {
...
function scaleArray(p: CalcParams, key: string, deltaPct: number) {
  if (Array.isArray(p[key])) {
    p[key] = p[key].map((v: number) => (typeof v === 'number' ? v * (1 + deltaPct / 100) : v));
  }
}

export function applyDefectDeltas(
  baseParams: CalcParams,
  defects: DefectKey[],
  machine: MachineLimits,
  material: MaterialRanges
): CalcParams {
  const p: CalcParams = { ...baseParams };
  const notes: string[] = [];
  p.notes = Array.isArray(p.notes) ? [...p.notes] : [];

  // Ordine di priorità: sicurezza → riempimento → dimensionale → estetica
  const order: DefectKey[] = [
    'burn', 'flash',
    'short_shot', 'jetting',
    'sink', 'voids', 'warpage',
    'weld_line', 'splay',
  ...
  for (const d of selected) {
    switch (d) {
      ...
      case 'sink':
        ...
        if (Array.isArray(p.packTimes_s)) p.packTimes_s = p.packTimes_s.map((t: number)=>t+0.7);
        addAbsolute(p, 'moldTemp_C', +5);
        addAbsolute(p, 'moldTempFixed_C', +3);
        addAbsolute(p, 'moldTempMoving_C', +3);
  ...
  if (notes.length) p.notes.push(`Correzioni difetti: ${notes.join(' · ')}`);
  return p;
}