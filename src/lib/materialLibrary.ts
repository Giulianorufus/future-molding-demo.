...
export type CrossWLF = {
  n: number;          // indice di shear thinning
  tauStar: number;    // Pa
  D1: number;         // Pa·s
  A1: number;
  A2: number;         // K
...
export const MATERIAL_LIBRARY: MaterialData[] = [
  {
    key: 'ABS_generic',
    name: 'ABS (generic)',
    density_gcm3: 0.94,
...
/* =============================================================================
   Future Molding – Guida tonnellaggio vs peso stampata
   Fonte: dati industriali indicativi (Rodon, Boyan Mfg)
   NB: valori empirici, da affinare su area proiettata reale
============================================================================= */
...
export const TONNAGE_GUIDE: TonnagePoint[] = [
 ...
 */
export function estimateTonnage(shot_g: number): number {
  for (let i = 0; i < TONNAGE_GUIDE.length; i++) {
    if (shot_g <= TONNAGE_GUIDE[i].shot_g) return TONNAGE_GUIDE[i].tonnage_t;
  }
