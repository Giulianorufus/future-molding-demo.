...
// Deriva parametri operativi dalla pressa selezionata.
// Input opzionali per adattare al pezzo/materiale.
export type DerivedFromPress = {
  recommendedClamp_kN: { min: number; max: number };
  // shot utile (consigliato 30–70% del max per stabilità)
  usefulShot_cm3: { min: number; max: number };
  // velocità target (in cm3/s) come percentuale della capacità macchina
  injSpeed_cm3s: { min: number; max: number };
  // pressione picco consigliata come % della max macchina
  injPressure_bar: { min: number; max: number };
  // cushion consigliato non inferiore al minimo macchina
  cushion_cm3: { min: number; target: number };
};

export function deriveFromPress(
  press: PressSpecs,
  opts?: { partVolume_cm3?: number; runnerVolume_cm3?: number; materialFactor?: number }
): DerivedFromPress {
