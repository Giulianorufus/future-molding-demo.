// pressMapping.ts
export interface PressConversionParams {
  pressBrand?: string;
  screwDiam_mm?: number;
  Vdot_cm3s?: number;
  Pc_cm3?: number;
  commutazione_cm3?: number;
  pressures_bar?: number[];
  k_trans?: number;
}

export interface PressConversionResult {
  velocita: number;
  velUnit: string;
  pc: number;
  pcUnit: string;
  ...
  pressioni: number[];
  pressUnit: string;
  tempoIniezione_s?: string;
}

export function convertForPress({
  pressBrand,        
  screwDiam_mm,      
  Vdot_cm3s,         
  Pc_cm3,            
  ...
  pressures_bar,     
  k_trans = 10       
}: PressConversionParams): PressConversionResult {
  const A = Math.PI * Math.pow((screwDiam_mm || 25) / 2, 2);
  const MM3_PER_CM3 = 1000;

  let out: PressConversionResult = {
    velocita: 0,
    velUnit: "cm³/s",
    pc: 0,
    pcUnit: "cm³",
  ...
  switch ((pressBrand || "").toLowerCase()) {
    ...
    case "sandretto":
      ...
      if (vdot > 0) out.tempoIniezione_s = (pc / vdot).toFixed(2);
      break;
