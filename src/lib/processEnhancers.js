 ...
 *  - default: 0.22
 */
function estimateGateFreezeTime_s(material, gateThickness_mm) {
...
function applyGateFreezeCap(p, material, cad, notes) {
    ...
    if (typeof p.packTime_s === 'number') {
        ...
        if (p.packTime_s < before)
            notes.push(`Pack time limitato al gate freeze ≈ ${round1(tFreeze)} s`);
    }
    else if (Array.isArray(p.packTimes_s) && p.packTimes_s.length) {
        const beforeMax = Math.max(...p.packTimes_s);
        p.packTimes_s = p.packTimes_s.map((t) => Math.min(t, round1(tFreeze)));
        const afterMax = Math.max(...p.packTimes_s);
        if (afterMax < beforeMax)
            notes.push(`Pack times limitati al gate freeze ≈ ${round1(tFreeze)} s`);
    }
    else {
 ...
 */
function targetCavityBar(material) {
    const name = (material?.name || material?.code || material?.tipo || '').toString().toUpperCase();
    if (/(PP|PE|PE-HD|PEHD)/.test(name))
        return 400;
    if (/(ABS|PS|PMMA)/.test(name))
        return 500;
    if (/(PC|PA|POM|GF|CF)/.test(name))
        return 550;
    return 500;
}
function applyClampFromProjectedArea(p, material, cad, pressa, notes) {
    const area_cm2 = cad?.projectedArea_cm2 || cad?.areaProjected_cm2 || null;
    if (!area_cm2 || !Number.isFinite(area_cm2))
        return;
 ...
 */
function estimateCoolingEfficiency(cad) {
    ...
    for (const ch of channels) {
        ...
        if (!Number.isFinite(d) || !Number.isFinite(dist) || d <= 0 || dist <= 0)
            continue;
        const ratio = dist / d;
        let eta = 1.0;
        if (ratio <= 1.0)
    ...
    if (!scores.length)
        return { eta: 0.75, reason: 'dati canali insufficienti (fallback)' };
    // prendi il peggiore (conservativo)
    const eta = Math.min(...scores);
    return { eta, reason: 'geometria canali' };
}
function applyCoolingEfficiency(p, cad, notes) {
    if (typeof p.coolingTime_s !== 'number')
        return;
 ...
 */
function suggestMoldTemps(material, cad) {
    ...
    const clamp = (x) => Math.round(clampNum(x, minR, maxR));
...
function applyMoldTemps(p, material, cad, notes) {
    const s = suggestMoldTemps(material, cad);
    if (!s)
        return;
    if (typeof p.moldTempFixed_C !== 'number')
        p.moldTempFixed_C = s.fixed;
    if (typeof p.moldTempMoving_C !== 'number')
        p.moldTempMoving_C = s.moving;
...
/* ----------------------------- Orchestratore unico ----------------------------- */
export function applyProcessEnhancers(result, ctx) {
    const p = { ...result };
    const notes = Array.isArray(p.notes) ? [...p.notes] : [];
    try {
        applyGateFreezeCap(p, ctx.material, ctx.cad, notes);
    }
    catch { }
    try {
        applyClampFromProjectedArea(p, ctx.material, ctx.cad, ctx.pressa, notes);
    }
    catch { }
    try {
        applyCoolingEfficiency(p, ctx.cad, notes);
    }
    catch { }
    try {
        applyMoldTemps(p, ctx.material, ctx.cad, notes);
    }
    catch { }
    if (notes.length)
        p.notes = notes;
