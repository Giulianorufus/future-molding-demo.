// Step di mantenimento: pressione e tempo
export type HoldingStep = {
  pressione: number; // bar
  tempo_s: number;   // secondi
  da_s: number;
  a_s: number;
};
...
export function suggestHoldingProfile({ volume_cm3, material, pressSpecs } : {
  volume_cm3: number;
  material: { name: string; viscosity: number };
  pressSpecs: { maxInjectionPressure_bar: number };
}): HoldingStep[] {
  // Esempio semplice: 3 tappe di mantenimento
  const steps: HoldingStep[] = [];
...
// Suggerisce un profilo multi-step di velocità e pressione per l'iniezione
// Input: volume, spessore, materiale, pressa
// Output: array di step [{ velocita, pressione, da_mm, a_mm }]

export type InjectionStep = {
  velocita: number; // cm3/s
  pressione: number; // bar
  da_mm: number;
  a_mm: number;
};
...
export interface MaterialInfo {
  tipo: "ABS" | "PC" | "PP" | "PA-GF" | "PE" | "PS" | "POM" | "PBT" | "TPU" | "PMMA";
  viscosita: "bassa" | "media" | "alta";
  shearSensitive: boolean;
}

export interface SuggestInjectionProfileParams {
  Vcar: number; // cm³
  Pc: number;   // cm³
  t_mm: number;
  materiale: MaterialInfo;
  estetico?: boolean;
  zoneSottili?: boolean;
  longRunner?: boolean;
  difettiStorico?: string[]; // es. ["striature","incompleto","jetting"]
  Vdot_max?: number; // cm³/s (esempio: prendi da macchina reale)
  Pmax?: number;     // bar
}

export interface InjectionProfileSuggestion {
  steps: number;
  switchesCm3: number[];
  speedsCm3s: number[];
  pressuresBar: number[];
...
export function suggestInjectionProfile({
  ...
  Pc,   // cm³
  t_mm,
  materiale, // { tipo: "ABS"|"PC"|"PP"|"PA-GF"|..., viscosita: "bassa|media|alta", shearSensitive: boolean }
  estetico = false,
  zoneSottili = false,
  longRunner = false,
  difettiStorico = [], // es. ["striature","incompleto","jetting"]
  Vdot_max = 200, // cm³/s (esempio: prendi da macchina reale)
  Pmax = 1800    // bar
}: SuggestInjectionProfileParams): InjectionProfileSuggestion {
  ...
  if (viscAlta) speeds = speeds.map(v => 0.9 * v);
  if (zoneSottili) speeds[0] = 1.1 * speeds[0];
  if (hasFlowIssues) speeds = speeds.map(v => 0.9 * v);
