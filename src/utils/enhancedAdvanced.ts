...
import type { EnhancedInputs, EnhancedOutput, MachineLimits, MaterialKey } from "./enhancedCalcEngine";

// 1) Shear heating correction (ΔT_eff)
function applyShearHeating(eta_PaS:number, gamma_s:number, baseTmelt_C:number): number {
...
function estimateResidenceTime_s(machine:MachineLimits, shotVol_cm3:number, screwLtoD=20): number {
...
function linePressureLoss_bar(hotRunner:boolean, runnerVol_cm3:number=0): number {
  let loss = hotRunner ? 40 : 80;
  if (!hotRunner) loss += Math.min(runnerVol_cm3*2, 120);
  return loss;
}

// 4) Startup ramp factors (primi 20 cicli)
function startupRamp(cycleCount:number, val:number, type:'pack'|'vel'): number {
  if (cycleCount < 20) {
...
function computeWarpRisk(partTh_mm:number, pack1_bar:number, material:MaterialKey, deltaT_percent:number): {risk:number; msg:string} {
  const gf = (material==='PA66_GF40')?1:0;
  const risk = 0.5*deltaT_percent + 0.002*pack1_bar + 15*gf;
  let msg='';
  if (risk>60) msg='Alto rischio imbarcamento: ridurre pack 2° stadio, alzare T stampo, bilanciare raffreddamento.';
  else if (risk>40) msg='Rischio medio imbarcamento: controllare simmetria raffreddamento e pack.';
  return {risk, msg};
}

// 6) Productivity & energy
function throughputAndEnergy(cycle_s:number, clamp_kN:number): {pph:number; kWh:number} {
...
export function enhanceAdvanced(enh:EnhancedOutput, inp:EnhancedInputs, cycleCount=100): EnhancedOutput {
  const out = {...enh};
  // Shear heating
...
// ===== Macchine generiche (profili) =====
export type MachineProfile = {
  key: string;                 // es. 'GEN-100T-Ø25'
  label: string;               // testo libero (non esposto in UI se non vuoi)
  clamp_t: number;             // tonnellaggio nominale
...
export function getMachineProfile(key: string): MachineProfile | undefined {
  return MACHINE_PROFILES.find(m => m.key === key);
}

// ===== Calibrazione da CSV (senza dipendenze) =====
export type CalibrationCoefficients = {
  kVisc: number;        // default 1.00
  dP_line_bar: number;  // default 80 (cold) / 40 (hot) – qui lo stimiamo dai dati
  kCool: number;        // default 1.00
  nRows: number;
  notes: string[];
};

export type CsvCalibrationRow = {
  // INPUT essenziali (quelli minimi per rifare il calcolo)
  material_key: string;          // es. 'PP_generic'
  machine_key: string;           // es. 'GEN-100T-Ø25'
  hot_runner: 'yes'|'no';
  part_thickness_mm: number;
  gate_type: 'tab'|'edge'|'pin'|'fan'|'submarine';
  gate_thickness_mm?: number;
  gate_width_mm?: number;
  gate_diameter_mm?: number;
  mold_temp_C: number;
  melt_temp_C: number;
  flow_length_mm: number;
  shot_volume_cm3: number;
  runner_volume_cm3?: number;
  inj_speed_cm3s: number;

  // MISURE DI REPARTO (target per calibrazione)
  measured_Pinj_bar: number;     // pressione di iniezione macchina (nozzle)
  measured_cooling_s: number;    // tempo raffreddamento effettivo
  measured_part_weight_g?: number; // opzionale (per controlli aggiuntivi)
};

export const CSV_TEMPLATE_HEADER =
`material_key,machine_key,hot_runner,part_thickness_mm,gate_type,gate_thickness_mm,gate_width_mm,gate_diameter_mm,mold_temp_C,melt_temp_C,flow_length_mm,shot_volume_cm3,runner_volume_cm3,inj_speed_cm3s,measured_Pinj_bar,measured_cooling_s,measured_part_weight_g`;

export const CSV_TEMPLATE_EXAMPLE_ROW =
`PP_generic,GEN-100T-Ø25,no,2.5,pin,, ,1.2,80,230,120,38,6,40,1020,14,22.4`;

/* ====================== Parser CSV minimale (no dipendenze) ====================== */
function parseCsv(text: string): CsvCalibrationRow[] {
  const lines = text.trim().split(/\r?\n/);
  if (lines.length < 2) return [];
  const header = lines[0].split(',').map(s => s.trim());
  const rows: CsvCalibrationRow[] = [];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const row: CsvCalibrationRow = {
      ...
      gate_type: (get('gate_type') as CsvCalibrationRow['gate_type']) ?? 'edge',
...
export function calibrateFromCSV(
  csvText: string,
  simulate: (row: CsvCalibrationRow) => { Pinj_bar: number; cooling_s: number }
  ...
  const pinjRatios: number[] = [];
  const coolingRatios: number[] = [];
  const losses: number[] = [];
  ...
  for (const r of rows) {
    ...
    if (loss_local > 0) losses.push(loss_local);
