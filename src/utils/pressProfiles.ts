...
type FieldKey =
  ...
  | "p_iniezione_bar"
  | "p_mantenimento_bar"
  | "tempo_mantenimento_s"
  | "contropress_bar"
  | "velocita_carica_rpm"
  | "risucchio_post_carica_mm"
  | "temp_cilindro_C"
  | "temp_stampo_C"
  | "tempo_raffreddamento_s"
  | "forza_chiusura_t"
  | "note_logica";
...
type FieldDef = {
  ...
  min?: number;
  max?: number;
  labelOverride?: string; // opzionale: NON cambia il markup, solo il testo
  unitOverride?: string;  // opzionale
};


type PressProfile = {
  fields: Partial<Record<FieldKey, FieldDef>>;
  fixed?: Partial<Record<FieldKey, number | string>>;
  notes?: string;
};
...
export function applyPressProfile(
  pressName: string,
  ...
  if (prof.fixed) {
    Object.entries(prof.fixed).forEach(([k, v]) => (next[k as FieldKey] = v));
  }
