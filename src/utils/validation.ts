import { CalcInputs, CalcOutputs, ValidationIssue } from "@/types/audit";
...
export function validateRun(inputs: CalcInputs, out: CalcOutputs): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (!inputs.pressId) issues.push({ kind: "missing", msg: "Pressa non selezionata", severity: "high" });
  if (!inputs.modelId) issues.push({ kind: "missing", msg: "Modello pressa non selezionato", severity: "high" });
